            ...
            <div class="card card-statistics h-100">
                <form id="invoice_form" action="{{ route('salesInvoice.store') }}" method="POST">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="invoice_date" name="invoice_date"
                                        value="{{ getCurrentDate() }}" class="form-control">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="invoice_number">رقم الفاتورة</label>
                                    <input type="text" value="{{ old('invoice_number', generateInvoiceNumber()) }}"
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <label for="invoice_note">ملاحظات الفاتورة</label>
                                <input type="text" id="invoice_note" name="note" class="form-control">
                            </div>

                            <div class="col-md-4">
                                <label for="payment_method" class="d-block">وسيلة الدفع</label>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="payment_method" id="payment_cash"
                                        value="cash" checked>
                                    <label class="form-check-label" for="payment_cash">كاش</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="payment_method" id="payment_visa"
                                        value="visa">
                                    <label class="form-check-label" for="payment_visa">فيزا</label>
                                </div>
                        ...
                        <div class="card-body mt-3">
                            <h4 style="font-family: 'Cairo', sans-serif;color: #84BA3F">بيانات الأصناف</h4>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $("#quantity").off("keypress").on("keypress", function(e) {
                                if (e.which === 13) {
                                    e.preventDefault();
                                    var barcode = $('#international_code')
                                        .val();
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            ...
                                            newRow +=
                                                ...
                                                barcode +
                                                '" class="form-control barcode" readonly></td>';
                                            newRow +=
                                                '<td><input type="text" name="itemName[]" value="' +
                                                name +
                                                '" class="form-control itemName" readonly></td>';
                                            newRow +=
                                                '<td hidden><input type="hidden" name="product_id[]" value="' +
                                                productId +
                                                '" class="form-control product_id" readonly></td>';
                                            newRow +=
                                                '<td><input type="number" name="quantity[]" value="' +
                                                quantity +
                                                '" class="form-control quantity" readonly></td>';
                                            newRow +=
                                                '<td><input type="number" name="sellingPrice[]" value="' +
                                                salePrice +
                                                '" class="form-control sellingPrice" readonly data-original-price="' +
                                                salePrice +
                                                '"></td>';
                                            newRow +=
                                                '<td><input type="number" name="totalValue[]" value="' +
                                                total +
                                                '" class="form-control totalValue" readonly></td>';
                                            newRow +=
                                                '<td><button type="button" class="btn btn-danger btn-sm deleteRow">حذف</button></td>';
                                            newRow += "</tr>";
                                            ...
                                            $("#international_code, #itemName, #quantity, #sellingPrice, #totalValue")
                                                .val("");
                                            $("#international_code")
                                                .focus();
                                        },
                ...
                $("#invoice_details_table tbody tr").each(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (parseFloat(salePriceInput.val()) == parseFloat(
                                    originalPrice)) {
